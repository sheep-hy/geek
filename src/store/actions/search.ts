import { ApiRsponse, Suggestion, SearchResult } from '@/types/data.d'
...
export function getSuggests(keyword: string): RootThunkAction {
  return async (dispatch) => {
    ...
    const res = await request.get<ApiRsponse<{ options: Suggestion }>>(
      '/suggestion',
      {
        params: {
          q: keyword,
...
// 搜索页面结果
export const getSearchResult = (keyword: string,page:number=1,per_page:number=10): RootThunkAction => {
  return async (dispatch) => {
    // 后端返回的类型 <ApiRsponse<Token>
    const res = await request.get<
      ApiRsponse<SearchResult>
    >('/search', {
      params: {
        q: keyword,
        page,
